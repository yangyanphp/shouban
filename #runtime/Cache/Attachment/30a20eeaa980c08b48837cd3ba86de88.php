...
<div class="wrap" style="padding:5px;">
  <div class="pop_nav">
    <ul class="J_tabs_nav">
      ...
      <?php if( $isadmin ): ?><li class=""><a href="javascript:;;" onClick="set_iframe('album_list','<?php echo U("Admin/public_album_load",array("args"=>$_GET['args']));?>')">图库</a></li><?php endif; ?>
      <?php if($att_not_used != ''): ?><li class="" ><a href="javascript:;;" style="color:#F00">未处理文件</a></li><?php endif; ?>
    </ul>
  </div>
  <div class="J_tabs_contents">
    <div class="pop_cont">
      <div class="">
        <div class="edit_uping"> 
            ...
            <span class="num"><input type="checkbox" id="watermark_enable" value="1" <?php if( $watermark_enable ): ?>checked<?php endif; ?>  onclick="change_params()"><em>是否添加水印</em> 最多上传<em> <?php echo ($file_upload_limit); ?></em> 个附件,单文件最大 <em><?php echo ($file_size_limit); ?> KB</em>，<em style="cursor: help;" title="可上传格式：<?php echo ($file_types); ?>">支持格式？</em></span>
        </div>
    ...
    <?php if($att_not_used != ''): ?><div class="pop_cont">
       <div class="eidt_uphoto">
          <ul class="cc" style="height: 330px;">
            <?php if(is_array($att)): $i = 0; $__LIST__ = $att;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li class="uploaded">
                <div class="get" id="aid-<?php echo ($vo["aid"]); ?>"> <a class="del" href="javascript:;">删除</a> 
                <img onClick="album_cancel(this,'<?php echo ($vo["aid"]); ?>','<?php echo ($vo["src"]); ?>')" width="87" height="98" src="<?php echo ($vo["fileimg"]); ?>" data-id="<?php echo ($vo["aid"]); ?>" data-path="<?php echo ($vo["src"]); ?>" alt="<?php echo ($vo["filename"]); ?>" title="<?php echo ($vo["filename"]); ?>"><input type="text" class="J_file_desc" name="flashatt[<?php echo ($vo["aid"]); ?>]" value="<?php echo ($vo["filename"]); ?>" style="width:68px"></div>
