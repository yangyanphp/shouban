...
<title>编辑：《<?php echo ($data["title"]); ?>》 - 系统后台 - <?php echo ($Config["sitename"]); ?> - by ShuipFCMS</title>
...
<div class="wrap J_check_wrap">
  <form name="myform" id="myform" action="<?php echo U("Content/edit");?>" method="post" class="J_ajaxForms" enctype="multipart/form-data">
  <div class="col-right">
    <div class="table_full">
      <table width="100%">
<?php
if(is_array($forminfos['senior'])) { foreach($forminfos['senior'] as $field=>$info) { if($info['isomnipotent']) continue; if($info['formtype']=='omnipotent') { foreach($forminfos['base'] as $_fm=>$_fm_value) { if($_fm_value['isomnipotent']) { $info['form'] = str_replace('{'.$_fm.'}',$_fm_value['form'],$info['form']); } } foreach($forminfos['senior'] as $_fm=>$_fm_value) { if($_fm_value['isomnipotent']) { $info['form'] = str_replace('{'.$_fm.'}',$_fm_value['form'],$info['form']); } } } ?>
         <tr>
...
<?php
        ...
        <tr>
          ...
          <span class="switch_list cc">
			<label><input type="radio" name="info[status]" value="99" <?php if( $data['status']==99 ): ?>checked<?php endif; ?>><span>审核通过</span></label>
			<label><input type="radio" name="info[status]" value="1"  <?php if( $data['status']==1 ): ?>checked<?php endif; ?>><span>待审核</span></label>
		 </span></td>
        </tr>
      </table>
    </div>
  ...
  <div class="col-auto">
    ...
    <div class="table_full">
      <table width="100%">
            <?php
if(is_array($forminfos['base'])) { foreach($forminfos['base'] as $field=>$info) { if($info['isomnipotent']) continue; if($info['formtype']=='omnipotent') { foreach($forminfos['base'] as $_fm=>$_fm_value) { if($_fm_value['isomnipotent']) { $info['form'] = str_replace('{'.$_fm.'}',$_fm_value['form'],$info['form']); } } foreach($forminfos['senior'] as $_fm=>$_fm_value) { if($_fm_value['isomnipotent']) { $info['form'] = str_replace('{'.$_fm.'}',$_fm_value['form'],$info['form']); } } } ?>
            <tr>
              <th width="80">
                <?php echo $info['name'];?> 
               </th>
              <td><?php if($info['star']){ ?><span class="must_red">*</span><?php } echo $info['form'];?> <span><?php echo $info['tips'];?></span></td>
            </tr>
            <?php
} } ?>
        </tbody>
...
<script type="text/javascript" src="<?php echo ($config_siteurl); ?>statics/js/common.js"></script>
...
$(function () {
    ...
    Wind.use('validate', 'ajaxForm', 'artDialog', function () {
        ...
        form.validate({
            ...
            //验证规则
            rules: <?php echo ($formValidateRules); ?>,
            //验证未通过提示消息
            messages: <?php echo ($formValidateMessages); ?>,
            //给未通过验证的元素加效果,闪烁等
            highlight: false,
            //是否在获取焦点时验证
            onfocusout: false,
							        ...
							        button:[
										{
											name: '添加一篇新文章？',
											callback:function(){
												window.location.href = "<?php echo U('Content/add',array('catid'=>$catid));?>";
												return true;
											},
											focus: true
										},{
...
function public_lock_renewal(){
    $.get("<?php echo U('Content/public_lock_renewal',array('catid'=>$catid,'id'=>$id));?>");
}
</script>
</body>
</html>